...
function loadSources(viewerOptions) {
    getDataSourceInfo().then(function (result) {
        if (result === undefined) {
            return;
        }

        // load source A
        loadSourceA(viewerOptions, result).then(function (isSourceALoaded) {
            // load source B
            loadSourceB(viewerOptions, result).then(function (isSourceBLoaded) {
            });
        });
    });
}
